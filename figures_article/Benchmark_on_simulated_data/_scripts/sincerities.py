...
def score_matrix(d, alpha=1, l1=1e-8, verb=False):
    ...
    if alpha is None:
        ...
        for i in range(n_alphas):
            ...
            if verb: print('{:.2e} -> {:.3f}'.format(valpha[i], verror[i]))
        vi = np.argmin(verror)
        if np.min(vi) == 0:
            if verb: print('Warning: the best alpha is reaching left bound')
        elif np.max(vi) == n_alphas - 1:
            if verb: print('Warning: the best alpha is reaching right bound')
