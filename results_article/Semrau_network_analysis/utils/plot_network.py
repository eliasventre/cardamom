...
# Option 0: basic colors
activ = 'green'
inhib = 'red'
# Option 1: pastel colors
activ = '#96D255'
inhib = '#E97356'
# Option 2: stronger colors
...
def node(k, ax, p, name, scale=1, color=None, fontsize=None, nodesize=1):
    ...
    #         zorder=4, horizontalalignment='center',
...
def link(k1, k2, ax, p, weight, bend=0, scale=1, nodesize=1, alpha=None):
    ...
    if weight > 0:
        ...
        connectionstyle='arc3,rad={}'.format(bend), alpha=alpha)
        ax.add_artist(arrow)
...
def plot_network(inter, width=1, height=1, scale=1, names=None, vdict=None,
    ...
    file=None, verb=False, fontsize=None, vcolor=None, nodesize=1, alpha=None):
    """
    Plot a network.
    """
    G, G = inter.shape
    ...
    if vcolor is None: vcolor = ['gray' for k in range(G)]
    ...
    for k1, k2 in e:
        ...
        if k1 != k2:
            if (k2, k1) in e:
                link(k1, k2, ax, p, weight, 0.1, scale, nodesize=nodesize,
                    alpha=alpha)
                # print(names[k1], names[k2])
