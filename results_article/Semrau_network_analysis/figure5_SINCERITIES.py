# Network inferred by SINCERITIES
import numpy as np
...
inter = np.load(path+'inferred_networks/SINCERITIES.npy') * 10
...
cnt = 0
seuil_min, seuil_max = 0.02, 5
theta[0,:] *= (np.abs(theta[0,:]) >= 0.02)
for j in range(1,G):
    for i in range(1,G):
        # Negative threshold
        if len(theta[theta[:,i]<0,i]):
            seuil_neg = np.quantile(-theta[theta[:,i]<0,i], .96)
        else: seuil_neg = seuil_min
        # Positive threshold
        if len(theta[theta[:,i]>0,i]):
            seuil_pos = np.quantile(theta[theta[:,i]>0,i], .96)
...
np.savetxt('sinc_net.txt',(theta*100).astype(int) ,delimiter='\t', fmt='% 4d')
...
for k1, k2 in zip(*theta.nonzero()):
    ...
    if weight_real != 0:
        ...
        s = 0.37
        if (k1,k2) == (0,29): s = 0.15
        if (k1,k2) == (0,16): s = 0.15
        if (k1,k2) == (0,18): s = 0.15
        if (k1,k2) == (0,12): s = 0.15
        if (k1,k2) == (0,8): s = 0.15
        if (k1,k2) == (0,30): s = 0.9
        if (k1,k2) == (0,31): s = 0.9
        if (k1,k2) == (0,13): s = 0.2
        if (k1,k2) == (0,14): s = 0.2
        if (k1,k2) == (0,15): s = 0.25
        if k1 == 1: s = 0.15
        if k1 == 6: s = 0.2
        if (k1,k2) == (6,8): s = 0.75
        if (k1,k2) == (1,2): s = 0.5
        if (k1,k2) == (1,34): s = 0.08
        if (k1,k2) == (2,11): s = 0.25
...
# Export the figure
fig.savefig('figure5_SINCERITIES.pdf', bbox_inches='tight', pad_inches=0.04)
